import { motion, AnimatePresence } from "framer-motion";
import { useEffect, useState } from "react";
import { XCircle } from "lucide-react";
import { toast } from "react-toastify";
import AdminWarrantyService from "../../../../service/AdminWarrantyService";

export default function WarrantySearchModal({
    isOpen,
    onClose,
    onSelectWarranty,
    initialWarranty
}) {
    const [query, setQuery] = useState("");

    const [warranties, setWarranties] = useState([]);
    const [total, setTotal] = useState(0);
    const [page, setPage] = useState(0);
    const pageSize = 10;

    const [loading, setLoading] = useState(false);

    const resetAll = () => {
        setQuery("");
        setWarranties([]);
        setTotal(0);
        setPage(0);
    };

    const loadWarranties = async (page = 0, keyword = null, append = false) => {
        setLoading(true);
        try {
            const res = await AdminWarrantyService.getWarranties({
                keyword: keyword || null,
                page,
                size: pageSize,
            });

            if (res.success) {
                if (append) {
                    setWarranties(prev => [...prev, ...(res.data.result || [])]);
                } else {
                    setWarranties(res.data.result || []);
                }
                setTotal(res.data.total || 0);
                setPage(page);
            } else {
                toast.error(res.message || "Không thể tải danh sách bảo hành");
            }
        } catch {
            toast.error("Lỗi khi tải danh sách bảo hành");
        }
        setLoading(false);
    };

    const loadMore = () => {
        if (warranties.length >= total) return;
        loadWarranties(page + 1, query.trim() !== "" ? query.trim() : null, true);
    };

    const handleKeyPress = (e) => {
        if (e.key === "Enter") {
            loadWarranties(0, query.trim() !== "" ? query.trim() : null, false);
        }
    };

    const clearInput = () => {
        setQuery("");
        loadWarranties(0, null, false);
    };

    const handleSelectWarranty = (warranty) => {
        onSelectWarranty(warranty);
        resetAll();
    };

    useEffect(() => {
        if (isOpen) {
            resetAll();

            if (initialWarranty) {
                // load highlight nếu cần
            }

            loadWarranties(0, null, false);
        }
    }, [isOpen, initialWarranty]);

    return (
        <AnimatePresence>
            {isOpen && (
                <>
                    {/* Overlay */}
                    <motion.div
                        className="fixed inset-0 bg-black z-40"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 0.4 }}
                        exit={{ opacity: 0 }}
                        onClick={onClose}
                    />

                    {/* Sidebar modal */}
                    <motion.div
                        className="fixed top-0 right-0 h-full w-[360px] bg-gray-900 shadow-xl z-50 p-6 overflow-auto"
                        initial={{ x: "100%" }}
                        animate={{ x: 0 }}
                        exit={{ x: "100%" }}
                        transition={{ type: "spring", stiffness: 300, damping: 30 }}
                    >
                        <h2 className="text-xl font-semibold mb-4 text-white">
                            Tìm kiếm Bảo hành
                        </h2>

                        {/* Search box */}
                        <div className="mb-4 relative">
                            <input
                                type="text"
                                value={query}
                                onChange={(e) => setQuery(e.target.value)}
                                onKeyDown={handleKeyPress}
                                className="w-full px-3 py-2 rounded bg-gray-800 text-white"
                                placeholder="Nhập tên bảo hành và Enter..."
                            />
                            {query && (
                                <button
                                    onClick={clearInput}
                                    className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white"
                                >
                                    <XCircle size={18} />
                                </button>
                            )}
                        </div>

                        {/* List */}
                        <div className="space-y-2 max-h-[70vh] overflow-auto">
                            {warranties.map(w => (
                                <div
                                    key={w.id}
                                    className="bg-gray-800 hover:bg-gray-700 text-white px-3 py-2 rounded cursor-pointer"
                                    onClick={() => handleSelectWarranty(w)}
                                >
                                    <div className="font-semibold">{w.name}</div>

                                    <div className="text-gray-400 text-xs mt-1">
                                        {w.description}
                                    </div>

                                    <div className="flex items-center gap-3 text-xs mt-1">
                                        <span className="text-blue-400">{w.months} tháng</span>
                                        <span className="text-green-400">
                                            {w.price > 0
                                                ? `${w.price.toLocaleString("vi-VN")} ₫`
                                                : "Miễn phí"}
                                        </span>
                                    </div>
                                </div>
                            ))}
                        </div>

                        {/* Load more */}
                        {total > warranties.length && (
                            <div className="mt-3 text-right">
                                <span
                                    className="text-blue-400 text-sm hover:underline cursor-pointer"
                                    onClick={loadMore}
                                >
                                    Xem thêm {total - warranties.length} bảo hành...
                                </span>
                            </div>
                        )}
                    </motion.div>
                </>
            )}
        </AnimatePresence>
    );
}

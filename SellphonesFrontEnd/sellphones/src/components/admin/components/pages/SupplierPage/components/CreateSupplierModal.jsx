...
export default function CreateSupplierModal({ isOpen, onClose, onCreate }) {
  ...
  const validate = () => {
    ...
    else if (!/^[\w.-]+@([\w-]+\.)+[\w-]{2,4}$/.test(formData.email)) newErrors.email = "Email không hợp lệ";
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (onCreate) await onCreate(newSupplier);
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <h2 className="text-xl font-semibold mb-6 text-white">Tạo nhà cung cấp mới</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                />
                {errors.taxCode && <div className="text-red-500 text-sm mt-1">{errors.taxCode}</div>}
              </div>
